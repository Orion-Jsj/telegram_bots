...
def init_db():
    with _write_lock, _conn() as c:
        ...
        rcols = [r[1] for r in c.execute("PRAGMA table_info(runs)").fetchall()]
        if "job_name" not in rcols:
            c.execute("ALTER TABLE runs ADD COLUMN job_name TEXT")
        if "kind" not in rcols:
            c.execute("ALTER TABLE runs ADD COLUMN kind TEXT DEFAULT 'clean'")
        jcols = [r[1] for r in c.execute("PRAGMA table_info(jobs)").fetchall()]
        if "kind" not in jcols:
            c.execute("ALTER TABLE jobs ADD COLUMN kind TEXT DEFAULT 'clean'")
    logbus.set_db_writer(_write_log)


# ---------- jobs ----------
def create_job(name, cfg, kind="clean"):
    with _write_lock, _conn() as c:
        cur = c.execute("INSERT INTO jobs(name, kind, config_json, created_at) VALUES (?,?,?,?)",
                        (name, kind, json.dumps(cfg), time.time()))
    ...
    with _conn() as c:
        rows = c.execute(
            """SELECT j.*,
               ...
               FROM jobs j WHERE j.kind=? ORDER BY j.created_at DESC""", (kind,)).fetchall()
        out = []
        for r in rows:
...
# ---------- runs ----------
def create_run(job_id, dry_run, job_name=None, kind="clean"):
    with _write_lock, _conn() as c:
        cur = c.execute(
            "INSERT INTO runs(job_id, job_name, kind, dry_run, status, started_at) VALUES (?,?,?,?,?,?)",
            (job_id, job_name, kind, 1 if dry_run else 0, "queued", time.time()))
        return cur.lastrowid
...
def list_runs(limit=200, kind="clean"):
    with _conn() as c:
        rows = c.execute(
            """SELECT r.*, COALESCE(r.job_name, j.name) AS display_name
               FROM runs r LEFT JOIN jobs j ON j.id=r.job_id
               WHERE r.kind=? ORDER BY r.started_at DESC LIMIT ?""", (kind, limit)).fetchall()
        out = []
        for r in rows:
            d = dict(r)
            d["detail"] = json.loads(d["detail_json"]) if d.get("detail_json") else {}
