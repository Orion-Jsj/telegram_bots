...
function startReportsTimer() {
  ...
  REPORTS_TIMER = setInterval(() => {
    ...
    const rep = document.querySelector('[data-panel="reports"]');
    const dl = document.querySelector('[data-panel="downloads"]');
    if (rep && !rep.classList.contains("hidden")) loadRunsTable();
    if (dl && !dl.classList.contains("hidden")) loadDlRuns();
  }, 5000);
}
...
async function deleteRun(r) {
  ...
  if (r.kind === "download") loadDlRuns(); else loadRunsTable();
}

// ================= DOWNLOADER =================
const DL_TYPES = [
  ["photos", "Photos", "img"], ["videos", "Videos", "vid"], ["files", "Files / docs", "doc"],
  ["audio", "Audio / music", "mp3"], ["voice", "Voice notes", "ogg"], ["gifs", "GIFs", "anim"],
];
let DL_DIALOGS = { groups: [], channels: [], dms: [] };
let DL_CHAT = null;
let DL_FILES = new Map();          // id -> entry
let DL_SELECTED = new Set();

function initBrowse() {
  // file-type checkboxes
  const wrap = $("#dlTypes"); wrap.innerHTML = "";
  DL_TYPES.forEach(([key, label, hint], idx) => {
    const on = ["photos", "videos", "files"].includes(key);
    const el = document.createElement("label");
    el.className = "chk" + (on ? " on" : "");
    el.innerHTML = `<input type="checkbox" data-k="${key}" ${on ? "checked" : ""}>
      <span class="lbl">${label}</span><span class="hint">${hint}</span>`;
    const cb = el.querySelector("input");
    cb.onchange = () => el.classList.toggle("on", cb.checked);
    wrap.appendChild(el);
  });
  $$("#dlTypes ~ * , .panel[data-panel='browse'] .subtab").forEach(() => {});
  $$(".panel[data-panel='browse'] .subtab").forEach((t) => (t.onclick = () => dlSwitchSub(t.dataset.bk)));
  $("#dlRefreshChats").onclick = () => dlLoadDialogs(true);
  $("#dlList").onclick = dlListFiles;
  $("#dlAll").onclick = () => dlSelectAll(true);
  $("#dlNone").onclick = () => dlSelectAll(false);
  $("#dlStart").onclick = dlStart;
  if (!$("#dlFolder").value) api("/api/dl/default-folder").then((d) => { if (d.folder) $("#dlFolder").value = d.folder; });
  dlLoadDialogs(false);
}

async function dlLoadDialogs(fresh) {
  $("#dlChatList").innerHTML = `<div class="item muted">loading…</div>`;
  const d = await api("/api/dialogs" + (fresh ? "?fresh=1" : ""));
  if (d.error) { $("#dlChatList").innerHTML = `<div class="item" style="color:var(--red)">${d.error}</div>`; return; }
  DL_DIALOGS = d; dlSwitchSub("groups");
}

function dlSwitchSub(k) {
  $$(".panel[data-panel='browse'] .subtab").forEach((t) => t.classList.toggle("active", t.dataset.bk === k));
  const list = k === "groups" ? DL_DIALOGS.groups : k === "channels" ? DL_DIALOGS.channels : DL_DIALOGS.dms;
  const wrap = $("#dlChatList"); wrap.innerHTML = "";
  if (!list.length) { wrap.innerHTML = `<div class="item muted">none</div>`; return; }
  list.forEach((c) => {
    const it = document.createElement("div");
    it.className = "item" + (DL_CHAT && DL_CHAT.id === c.id ? " sel" : "");
    it.innerHTML = `<span class="nm">${escapeHtml(c.title)}</span>
      <span class="meta">${c.username ? "@" + c.username + " · " : ""}${c.kind} · ${c.id}</span>`;
    it.onclick = () => {
      DL_CHAT = c;
      $$("#dlChatList .item").forEach((x) => x.classList.remove("sel"));
      it.classList.add("sel");
      $("#dlChosen").textContent = "chat: " + c.title;
    };
    wrap.appendChild(it);
  });
}

async function dlListFiles() {
  if (!DL_CHAT) return alert("Select a chat first.");
  const types = $$("#dlTypes input:checked").map((cb) => cb.dataset.k);
  if (!types.length) return alert("Pick at least one file type.");
  const limit = parseInt($("#dlLimit").value) || 500;
  const box = $("#dlFiles");
  box.innerHTML = `<div class="item muted">listing files…</div>`;
  const d = await api(`/api/dl/files?chat=${DL_CHAT.id}&types=${types.join(",")}&limit=${limit}`);
  if (d.error) { box.innerHTML = `<div class="item" style="color:var(--red)">${d.error}</div>`; return; }
  DL_FILES = new Map(); DL_SELECTED = new Set();
  if (!d.files.length) { box.innerHTML = `<div class="item muted">no files found for those types</div>`; $("#dlSummary").textContent = ""; return; }
  box.innerHTML = "";
  d.files.forEach((f) => {
    DL_FILES.set(f.id, f);
    const it = document.createElement("label");
    it.className = "item";
    it.innerHTML = `<input type="checkbox">
      <span class="nm">${escapeHtml(f.name)}</span>
      <span class="meta">${f.type} · ${humanSize(f.size)} · ${f.date}</span>`;
    const cb = it.querySelector("input");
    cb.onchange = () => { cb.checked ? DL_SELECTED.add(f.id) : DL_SELECTED.delete(f.id); it.classList.toggle("sel", cb.checked); dlSummary(); };
    box.appendChild(it);
  });
  dlSummary(d);
}
...
function dlSelectAll(on) {
  $$("#dlFiles .item").forEach((it, i) => {
    const cb = it.querySelector("input"); if (!cb) return;
    cb.checked = on; it.classList.toggle("sel", on);
  });
  DL_SELECTED = on ? new Set(DL_FILES.keys()) : new Set();
  dlSummary();
}

function dlSummary(d) {
  const total = DL_FILES.size;
  let selBytes = 0; DL_SELECTED.forEach((id) => (selBytes += (DL_FILES.get(id) || {}).size || 0));
  $("#dlSummary").textContent = `${DL_SELECTED.size}/${total} selected · ${humanSize(selBytes)}`;
}

async function dlStart() {
  if (!DL_CHAT) return alert("Select a chat.");
  if (!DL_SELECTED.size) return alert("Select at least one file.");
  const folder = $("#dlFolder").value.trim();
  const types = $$("#dlTypes input:checked").map((cb) => cb.dataset.k);
  const body = { chat: DL_CHAT.id, title: DL_CHAT.title, folder, types,
                 msg_ids: [...DL_SELECTED], name: `download · ${DL_CHAT.title}` };
  const r = await api("/api/dl/start", { method: "POST", body });
  const msg = $("#dlMsg");
  if (r.ok) { msg.style.color = "var(--teal)"; msg.textContent = "started ✓"; location.hash = "downloads"; loadDlRuns(); }
  else { msg.style.color = "var(--red)"; msg.textContent = r.error || "failed"; }
}

async function loadDlRuns() {
  const tb = $("#dlRunRows"); if (!tb) return;
  const runs = await api("/api/dl/runs");
  if (!runs.length) { tb.innerHTML = `<tr><td colspan="7" class="muted" style="padding:18px">No downloads yet. <a href="#browse" style="color:var(--amber)">Browse files</a>.</td></tr>`; return; }
  tb.innerHTML = "";
  runs.forEach((r) => {
    const total = r.scanned || 0, done = r.matched || 0;
    const pct = total ? Math.round((done / total) * 100) : 0;
    const folder = (r.detail && r.detail.folder) || "";
    const when = r.started_at ? new Date(r.started_at * 1000).toLocaleString() : "—";
    const stBadge = r.status === "paused" ? `<span class="badge paused">paused</span>`
      : r.status === "running" || r.status === "queued" ? `<span class="badge">${r.status}</span>`
      : r.status === "error" ? `<span class="badge live">error</span>`
    ...
    tr.innerHTML = `
      <td>#${r.id}</td>
      <td class="nm">${escapeHtml(r.job_name || "—")}</td>
      <td class="mono" style="font-size:10px">${escapeHtml(folder)}</td>
      <td><div class="progress"><i style="width:${pct}%"></i></div>
          <span class="mono" style="font-size:10px">${done}/${total}</span></td>
      <td>${stBadge}</td>
      <td class="mono" style="font-size:11px">${when}</td>
      <td class="actions"></td>`;
    const act = tr.querySelector(".actions");
    if (r.status === "paused" || r.status === "error") {
      const res = document.createElement("button"); res.className = "iconbtn dry"; res.textContent = "resume";
      res.onclick = (e) => { e.stopPropagation(); resumeRun(r); };
      act.appendChild(res);
    }
    ...
    del.onclick = (e) => { e.stopPropagation(); deleteRun(r); };
    act.appendChild(del);
    tr.onclick = () => showDlModal(r);
    tb.appendChild(tr);
  });
}

function showDlModal(r) {
  const files = (r.detail && r.detail.files) || [];
  const total = r.scanned || files.length, done = r.matched || 0;
  const rows = files.map((f) => `<tr>
    <td class="nm">${escapeHtml(f.name || ("msg " + f.id))}</td>
    <td>${f.type || "-"}</td>
    <td class="mono" style="font-size:11px">${humanSize(f.size)}</td>
    <td><span class="fstatus ${f.status}">${f.status}</span>${f.error ? ' <span class="muted" style="font-size:10px">' + escapeHtml(f.error) + "</span>" : ""}</td>
  </tr>`).join("");
  const body = `
    <div class="kv" style="margin-bottom:14px">
      <div class="k">folder</div><div class="mono" style="font-size:12px">${escapeHtml((r.detail && r.detail.folder) || "")}</div>
      <div class="k">progress</div><div>${done} / ${total} files</div>
    </div>
    <table><thead><tr><th>file</th><th>type</th><th>size</th><th>status</th></tr></thead>
    <tbody>${rows || '<tr><td colspan="4" class="muted">no files</td></tr>'}</tbody></table>`;
  const foot = [];
  if (r.status === "paused" || r.status === "error") foot.push(mkBtn("Resume", "safe", () => { closeModal(); resumeRun(r); }));
  foot.push(mkBtn("Delete", "danger", () => { closeModal(); deleteRun(r); }));
  foot.push(mkBtn("Close", "ghost", closeModal));
  openModal(`Download #${r.id} · ${escapeHtml(r.job_name || "")}`, body, foot);
}
...
// ================= tab router (single page, two modes) =================
const PANELS = ["home", "create", "jobs", "reports", "login", "browse", "downloads"];
const TAB_OF_MODE = { clean: ["home", "create", "jobs", "reports"], download: ["browse", "downloads"] };
const INITED = {};
let CURRENT_MODE = "clean";

function panelMode(name) {
  for (const m in TAB_OF_MODE) if (TAB_OF_MODE[m].includes(name)) return m;
  return CURRENT_MODE;   // login (shared) stays in current mode
}
...
function applyMode(mode) {
  ...
  $$(".tab").forEach((t) => {
    ...
    t.classList.toggle("hidden", !(m === mode || m === "both"));
  });
}

function showTab(name) {
  if (!PANELS.includes(name)) name = "home";
  if (name !== "login") applyMode(panelMode(name));
  PANELS.forEach((p) => {
  ...
  else if (name === "browse" && !INITED.browse) { initBrowse(); INITED.browse = true; }
  else if (name === "jobs") loadJobsTable();
  else if (name === "reports") loadRunsTable();
  else if (name === "downloads") loadDlRuns();
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  $$(".modebtn").forEach((b) => (b.onclick = () => { applyMode(b.dataset.mode); showTab(TAB_OF_MODE[b.dataset.mode][0]); }));
